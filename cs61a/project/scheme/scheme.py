...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        toReturn = scheme_eval(first, env)
        check_procedure(toReturn)
        arguments = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(toReturn, arguments, env)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        """
        child = Frame(self)
        if len(formals) is not len(vals):
            raise SchemeError()
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        check_form(current, 2, 2)
        if not scheme_symbolp(current.first):
            raise SchemeError
        else:
            formals = Pair(current.first, formals)
            vals = Pair(scheme_eval(current.second.first, env), vals)
