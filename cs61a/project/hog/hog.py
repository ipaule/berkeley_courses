...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say(score0, score1):
        ...
        if(pscore - ps > 0):
            if((pscore - ps) > ph):
                print(pscore - ps, "point(s)! That's the biggest gain yet for Player", whom)
                ph = pscore - ps
        ps = pscore
        return announce_highest(whom, ph, ps)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    while(i <= 10):
        ...
        if(max_avr < averaged_roll(i,dice)):
            max_avr = averaged_roll(i,dice)
            max_rolls = i
        i += 1
    return max_rolls
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    no_roll = score + free_bacon(opponent_score)
    if(is_swap(no_roll, opponent_score) & (no_roll < opponent_score)):
        return 0
    else:
