...
class Ant(Insect):
    ...
    def can_contain(self, other):
        return self.is_container and not other.is_container and self.contained_ant is None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        transition = 0
        while place != hive:
            if len(place.bees) is not 0:
                if self.min_range <= transition and transition <= self.max_range:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        if self.digesting <= 0:
            if len(self.place.bees) is not 0:
        ...
        while place is not None:
            ...
            if place.ant is not None:
                if place.ant.is_container:
                    if place.ant.contained_ant is not None:
                        if not place.ant.contained_ant.doubled:
                            place.ant.contained_ant.damage *= 2
                            place.ant.contained_ant.doubled = True
            place = place.exit
